#!/bin/bash
# Script to deploy web app and database for ClientA
...
STORAGE_ACCOUNT="stglexsbclienta"
...
# Artifacts
LOCAL_WEBAPP_ZIP="./lexdemo_202412261902.zip"
...
afd_url=$(az afd endpoint list -g rg-central --profile-name globalFrontDoor \
  --query "[?name=='afd-ep-lexsb-ClientA'].hostName" -o tsv)
...
curl -s -o /dev/null -w "%{http_code}" https://afd-ep-lexsb-ClientA-f0f6huhhecgtc9ep.z03.azurefd.net/
if [ $? -eq 0 ]; then
...
echo -e "${GREEN}Front Door URL: https://afd-ep-lexsb-ClientA-f0f6huhhecgtc9ep.z03.azurefd.net${NC}" 
